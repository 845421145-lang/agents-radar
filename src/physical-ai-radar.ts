...
async function main(): Promise<void> {
  ...
  console.log(`[physical-ai] ${signals.length} raw signals; ${gate.kept.length} passed deterministic physical-AI gate`);
  ...
  const llmInput = [...gate.kept].sort((a, b) => Number(Boolean(b.traction_signal)) - Number(Boolean(a.traction_signal))).slice(0, config.radar.max_prefiltered_for_llm);
  ...
  const scored = unique.map((candidate) => scoreCandidate(candidate, profile, { validate: config.radar.validate_threshold, watch: config.radar.watch_threshold }));
  const merged = mergeHistory(loadHistory(), scored, date);
  saveHistory(merged.store);
  const metrics = { raw_signals: signals.length, prefiltered: gate.kept.length, physical_candidates: merged.candidates.length, watch: merged.candidates.filter((candidate) => candidate.status === "WATCH").length, validate: merged.candidates.filter((candidate) => candidate.status === "VALIDATE").length, filtered: signals.length - gate.kept.length };
  const report = buildPhysicalAiReport(date, metrics, merged.candidates, sourceResults.map((result) => result.health));
  savePhysicalAiOutputs(date, report, merged.candidates, sourceResults.map((result) => result.health), signals);
  console.log(`[physical-ai] Done: ${metrics.validate} VALIDATE, ${metrics.watch} WATCH. Saved digests/${date}/physical-ai-radar.md`);
}
