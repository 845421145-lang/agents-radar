...
const inferred = <T>(field: { value: T | null; status: string; rationale: string | null }, formatter: (value: T) => string = String) => field.value === null ? "UNKNOWN（当前无证据）" : `${formatter(field.value)} [${field.status}]`;
...
export function buildPhysicalAiReport(date: string, metrics: RadarMetrics, candidates: OpportunityCandidate[], health: SourceHealth[]): string {
  ...
  const top = [...validate, ...watch].sort((a, b) => (b.score?.final_score ?? 0) - (a.score?.final_score ?? 0));
  const best = top[0];
  const limitations = health.filter((item) => item.status !== "ok").map((item) => `- **${item.source}**：${item.status}${item.limitation ? ` — ${item.limitation}` : ""}`);
...
export function savePhysicalAiOutputs(date: string, report: string, candidates: OpportunityCandidate[], health: SourceHealth[], signals: ProductSignal[]): void {
  ...
  fs.writeFileSync(path.join(dir, "opportunities.json"), JSON.stringify({ schema_version: "oomwoo.opportunity-candidate.v1", generated_at: new Date().toISOString(), candidates }, null, 2) + "\n");
  fs.writeFileSync(path.join(dir, "source-health.json"), JSON.stringify({ schema_version: "oomwoo.source-health.v1", sources: health }, null, 2) + "\n");
  fs.writeFileSync(path.join(dir, "normalized", "product-signals.json"), JSON.stringify({ schema_version: "oomwoo.product-signal.v1", signals }, null, 2) + "\n");
}
