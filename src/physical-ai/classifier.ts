...
const inferred = <T>(value: T | null, rationale: string | null): InferredField<T> => ({
  value,
  status: value === null ? "UNKNOWN" : "INFERENCE",
  rationale,
});
...
export function deterministicCandidate(
  ...
  method: OpportunityCandidate["classification_method"] = "deterministic",
  ...
  return {
    id: hash(canonical || signal.id),
    product_name: signal.title,
    canonical_name: canonical || signal.id,
    product_category: inferred(detail.category, "由标题/描述中的实体产品类别关键词推断。"),
    physical_product: {
      value: physical,
      status: "INFERENCE",
      rationale: "由公开标题/描述中的硬件关键词判定，仍需人工复核。",
    },
    ai_core_value: {
      value: ai,
      status: "INFERENCE",
      rationale: "由公开标题/描述中的 AI 关键词判定，尚未证明 AI 是核心而非营销。",
    },
    company: signal.company,
    region: unknown<string>(),
    price: signal.price,
    price_range: unknown<string>(),
    traction_signal: signal.traction_signal,
    market_signal: inferred(
      signal.traction_signal ? "公开平台互动/众筹信号存在" : "公开新品或行业报道信号",
      signal.traction_signal ? "来源直接提供互动或众筹字段。" : "仅基于新品发现来源。",
    ),
    problem_solved: unknown<string>(),
    target_customer: unknown<string>(),
    ai_value_proposition: unknown<string>(),
    manufacturing_requirements: inferred(detail.requirements, "根据产品类别映射到能力档案所需能力。"),
    supply_chain_signal: unknown<string>(),
    competition_signal: unknown<string>(),
    novelty_signal: inferred("新发现信号", "首次被当日来源捕获；不等同于市场首发。"),
    evidence: signal.evidence,
    sources: [signal.source],
    confidence: physical && ai ? 0.38 : 0.15,
...
function applyLlm(base: OpportunityCandidate, item: LlmClassification): OpportunityCandidate {
  if (!item.physical_product || !item.ai_core_value)
    return {
      ...base,
      physical_product: inferred(Boolean(item.physical_product), "LLM 基于给定信号的保守分类。"),
      ai_core_value: inferred(Boolean(item.ai_core_value), "LLM 基于给定信号的保守分类。"),
      confidence: Math.min(0.45, Math.max(0.1, item.confidence || 0.2)),
      classification_method: "llm",
      status: "REJECTED",
    };
  return {
    ...base,
    physical_product: inferred(true, "LLM 基于给定公开信号判定；需要后续验证。"),
    ai_core_value: inferred(true, "LLM 基于给定公开信号判定；需要后续验证。"),
    product_category: inferred(item.product_category, "LLM 基于给定公开信号归类。"),
    problem_solved: inferred(item.problem_solved, "LLM 推断，非来源直接事实。"),
    ...
    confidence: Math.min(
      0.85,
      Math.max(base.confidence, Number.isFinite(item.confidence) ? item.confidence : base.confidence),
    ),
    classification_method: "llm",
    next_validation_questions: item.next_validation_questions?.slice(0, 4) ?? base.next_validation_questions,
  };
}
...
export async function classifySignals(
  ...
  if (!useLlm || signals.length === 0)
    return bases.filter((candidate) => candidate.physical_product.value && candidate.ai_core_value.value);
  ...
  for (let start = 0; start < signals.length; start += batchSize) {
    ...
    try {
      ...
      for (const signal of batch) {
        const base =
          byId.get(hash(canonicalize(signal.title) || signal.id)) ??
          deterministicCandidate(signal, "fallback");
        const item = classified.get(signal.id);
        const candidate = item
          ? applyLlm(base, item)
          : {
              ...base,
              classification_method: "fallback" as const,
      ...
      results.push(
        ...batch
          .map((signal) => deterministicCandidate(signal, "fallback"))
          .filter((candidate) => candidate.physical_product.value && candidate.ai_core_value.value),
      );
    }
  }
  return results;
...
export function deduplicateCandidates(candidates: OpportunityCandidate[]): OpportunityCandidate[] {
  ...
  for (const candidate of candidates)
    groups.set(candidate.canonical_name, [...(groups.get(candidate.canonical_name) ?? []), candidate]);
  return [...groups.values()].map((group) => {
    ...
    return {
      ...
      evidence,
      sources,
      confidence: Math.min(0.9, first.confidence + Math.min(0.18, (sources.length - 1) * 0.08)),
      traction_signal:
        group.find((candidate) => candidate.traction_signal)?.traction_signal ?? first.traction_signal,
    };
  });
}
