...
export const PRODUCT_CATEGORY_HINTS: Array<[RegExp, string, string[]]> = [
  [
    /label|printer/i,
    "AI 标签/打印设备",
    ["plastics_injection", "pcb_electronics", "embedded_firmware", "final_assembly"],
  ],
  [
    /glasses|eyewear|smart glasses/i,
    "AI 眼镜/可穿戴",
    ["plastics_injection", "pcb_electronics", "sensors", "embedded_firmware", "final_assembly"],
  ],
  [
    /earbud|headphone|audio|speaker/i,
    "AI 音频穿戴",
    ["plastics_injection", "pcb_electronics", "sensors", "embedded_firmware", "final_assembly"],
  ],
  [
    /robot|robotic/i,
    "机器人",
    ["motors_actuators", "sensors", "pcb_electronics", "embedded_firmware", "final_assembly", "testing"],
  ],
  [
    /camera|vision|inspection/i,
    "AI 视觉/检测设备",
    ["sensors", "pcb_electronics", "embedded_firmware", "ai_software_integration", "testing"],
  ],
  [
    /home|thermostat|appliance|pet|toy/i,
    "AI 家庭/宠物/玩具设备",
    ["plastics_injection", "pcb_electronics", "sensors", "embedded_firmware", "final_assembly"],
  ],
  [
    /industrial|factory|machine|tool/i,
    "AI 工业设备/智能工具",
    ["sheet_metal_welding", "cnc_machining", "pcb_electronics", "sensors", "embedded_firmware", "testing"],
  ],
];
...
export function textForSignal(input: {
  title: string;
...
export function isPhysicalAiLikely(input: {
  title: string;
  description: string | null;
  raw_category: string | null;
}): boolean {
...
export function inferCategory(input: {
  ...
  return match
    ? { category: match[1], requirements: match[2] }
    : {
        category: "其他 Physical AI 设备",
        requirements: ["pcb_electronics", "embedded_firmware", "final_assembly"],
      };
}

export function hasAiSignal(input: {
  title: string;
  description: string | null;
  raw_category: string | null;
}): boolean {
